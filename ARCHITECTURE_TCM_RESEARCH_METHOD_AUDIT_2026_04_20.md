...
测试基线（2026-04-21 更新）：3022 passed / 0 failed / 4 xfailed / 2 skipped / 175 warnings。
...
- 范围：
  - [ ] 新建 `src/storage/graph_schema.py`，定义 `GRAPH_SCHEMA_VERSION`、标签、关系类型、属性白名单
  - [ ] 在 `src/storage/neo4j_driver.py` 增加 schema bootstrap、`get_schema_version()`、`ensure_schema_version()`
  - [ ] 让 `src/research/phase_orchestrator.py`、`src/research/research_session_graph_backfill.py` 改为通过 registry 取 label / relationship
  - [ ] 在 `src/api/routes/analysis.py` 暴露 `schema_version`、`schema_drift_detected`
- 完成定义：
  - [ ] 启用 Neo4j 后可读取 schema version，version mismatch 会给出 drift 报告
  - [ ] session / phase / artifact / observe 投影不再直接硬编码标签与关系类型
- 测试：
  - [ ] `tests/unit/test_graph_schema_versioning.py`
  - [ ] `tests/unit/test_neo4j_driver.py`
  - [ ] `test_kg_e2e.py`
...
- 范围：
  - [ ] 在 `src/storage/graph_schema.py` 注册 `Hypothesis`、`Evidence`、`EvidenceClaim` 及核心关系类型
  - [ ] 在 `src/research/phases/hypothesis_phase.py` 输出 `results["graph_assets"]["hypothesis_subgraph"]`
  - [ ] 在 `src/research/phases/analyze_phase.py` 基于 `src/research/evidence_contract.py` 生成 evidence / claim 子图
  - [ ] 由 `src/research/phase_orchestrator.py` 与 `src/storage/backend_factory.py` 统一承接图写入事务
- 完成定义：
  - [ ] hypothesis_count > 0 时必有 hypothesis subgraph node / edge 统计
  - [ ] 每条 claim 可回链到 evidence 节点，且图写入失败不会让 PG 事务误提交
- 测试：
  - [ ] `tests/unit/test_graph_asset_subgraphs.py`
  - [ ] `tests/unit/test_architecture_regression_guard.py`（Guard #36）
  - [ ] `test_kg_e2e.py`
...
- 范围：
  - [ ] 在 `src/storage/graph_schema.py` 注册 `Catalog`、`ExegesisTerm`、`FragmentCandidate`、`TextualEvidenceChain` 及其关系类型
  - [ ] 让 `src/research/observe_philology.py`、`src/analysis/philology_service.py` 输出标准化 philology graph assets
  - [ ] 把 `src/research/review_workbench.py` 的 `review_status`、`needs_manual_review`、`decision_basis` 同步进图节点属性
  - [ ] 在 `tools/neo4j_query_templates.py` 与 `src/api/routes/analysis.py` 增加 philology asset graph 查询模板
- 完成定义：
  - [ ] 存在 philology summary 时，可查询 `catalog -> witness -> fragment / exegesis` 链路
  - [ ] 图节点属性可直接筛选 accepted / rejected / needs_source / pending
- 测试：
  - [ ] `tests/unit/test_philology_graph_projection.py`
  - [ ] `tests/unit/test_neo4j_query_governance.py`
  - [ ] `tests/unit/test_textual_evidence_chain.py`
  - [ ] `tests/unit/test_catalog_contract.py`
...
- 范围：
  - [ ] 扩展 `src/research/research_session_graph_backfill.py`，为旧 cycle 生成 schema v2 资产
  - [ ] 为 `.vscode/production-local-backfill.ps1` 增加 graph schema version 与 dry-run 摘要
  - [ ] 新增图回归守卫，覆盖 schema registry、核心标签计数、必备关系类型、KG stats 字段
  - [ ] 扩展 `test_kg_e2e.py`，验证 hypothesis / evidence / philology 三类资产级查询
- 完成定义：
  - [ ] 旧 session 执行 backfill 后新增资产节点 / 边数量 > 0，且不破坏原有投影
  - [ ] 预检输出可见 schema version、预计新增节点数、预计新增边数
- 测试：
  - [ ] `tests/unit/test_graph_asset_regression.py`
  - [ ] `tests/unit/test_architecture_regression_guard.py`（Guard #37）
  - [ ] `test_kg_e2e.py`
...
- 范围：
  - [ ] 在 `src/api/research_utils.py`、`src/research/observe_philology.py` 输出 `review_queue`、`queue_summary`、`queue_filters`
  - [ ] 扩展 `src/api/routes/research.py`、`src/api/schemas.py`，支持按 `asset_type`、`review_status`、`priority_bucket`、`reviewer` 过滤 queue
  - [ ] 让 `web_console/static/index.html` 与 `src/web/routes/dashboard.py` 支持多选、全选、批量状态切换、批量备注
  - [ ] 为 `ResearchBatchPhilologyReviewRequest` 增加 `selection_snapshot`、共享备注与批处理审计字段
- 完成定义：
  - [ ] dashboard payload 中存在可消费的 `review_queue` 与统计摘要
  - [ ] 用户可对当前筛选结果执行一键批量 accepted / rejected / needs_source / pending 写回
- 测试：
  - [ ] `tests/unit/test_review_queue_contract.py`
  - [ ] `tests/test_research_utils.py`
  - [ ] `tests/test_web_console_api.py`
  - [ ] `tests/unit/test_dashboard_copy.py`
...
- 范围：
  - [ ] 在 `src/storage/db_models.py`、`src/infrastructure/persistence.py`、`alembic/versions/` 增加 review assignment 持久化模型
  - [ ] 在 `src/infrastructure/research_session_repo.py` 增加 claim / release / reassign / list_queue / aggregate_workload
  - [ ] 在 `src/api/routes/research.py`、`src/api/schemas.py` 增加 assignment / workload API
  - [ ] 在 `src/api/research_utils.py`、`src/web/routes/dashboard.py`、`web_console/static/index.html` 增加“我负责 / 无人认领 / 超期项”视图
- 完成定义：
  - [ ] PG 中可查询 review assignment 记录，不再只能从 artifact 反推 assignee
  - [ ] dashboard 卡片能展示 assignee、backlog age、priority
- 测试：
  - [ ] `tests/test_research_session_repo.py`
  - [ ] `tests/unit/test_reviewer_workload_board.py`
  - [ ] `tests/test_web_console_api.py`
...
| --- | --- | --- | --- |
| passed | 2893 | 3022 | +129（含 Phase F/G/I 新增测试） |
...
- [x] F-4-4：回归基线已文档化于此（3022 pass / 4 xfail / 2 skip）✓
...
1. 运行全量回归确认基线：`.\venv310\Scripts\activate; python -m pytest tests/ --tb=short -q`
2. 确认 3022+ pass / 0 fail / 4 xfail / 2 skip
3. 按 Phase G-1 -> G-4 -> H-1 -> I-2 顺序继续推进
