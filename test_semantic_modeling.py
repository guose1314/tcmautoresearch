...
def _print_modeling_banner() -> None:
    print("\n" + "=" * 80)
    print("语义建模升级演示（君臣佐使关系识别）")
    print("=" * 80)


def _preprocess_source_text(test_text: str):
    print("\n【第1步】文本预处理")
    print("-" * 80)
...
def _extract_modeling_entities(context, processed_text: str):
    print("\n【第2步】实体抽取")
    print("-" * 80)
...
def _build_semantic_graph(context, entities):
    print("\n【第3步】语义图构建 - 君臣佐使关系识别")
    print("-" * 80)
    ...
    return graph_result, nodes, edges, stats


def _print_relationship_details(edges, stats):
    print("\n【第4步】关系详解 - 君臣佐使分类")
    print("-" * 80)
...
def _print_formula_validation() -> None:
    print("\n【第5步】君臣佐使组成验证")
    print("-" * 80)
    for formula_name in ["补中益气汤", "四君子汤", "六味地黄丸"]:
...
def _print_relationship_catalog() -> None:
    print("\n【第6步】所有关系类型清单")
    print("-" * 80)
    print("\n  支持的关系类型：")
...
def _print_graph_samples(nodes, edges) -> None:
    print("\n【第7步】知识图谱概览")
    print("-" * 80)
    print("\n  【节点样本】（前10个）:")
...
def _print_modeling_summary(processed_text: str, entities, stats, relationships_by_type) -> None:
    print("\n" + "=" * 80)
...
def test_semantic_modeling():
    ...
    临床可用于防治各类虚弱性疾病。
    """
    context, processed_text = _preprocess_source_text(test_text)
    entities = _extract_modeling_entities(context, processed_text)
    graph_result, nodes, edges, stats = _build_semantic_graph(context, entities)
    relationships_by_type = _print_relationship_details(edges, stats)
    _print_formula_validation()
    _print_relationship_catalog()
    _print_graph_samples(nodes, edges)
    _print_modeling_summary(processed_text, entities, stats, relationships_by_type)
