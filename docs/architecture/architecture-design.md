...
> 历史设计文档（2026-03-30）：除后续同步说明外，本文中的“当前/现有/缺失/无 API 服务层/无持久化层”等判断均指 2026-03-30 的观察基线，不代表 2026-04-14 的仓库现态。当前实现状态应优先以较新的架构审计、模块合同、运行时装配与真实回归结果为准。
> **版本**: 3.0-Draft  
> **日期**: 2026-03-30  
...
- 本文里“无持久化层”“阶段 2 用 SQLite”“阶段 3 平台化后切 PostgreSQL”等判断，均属于 2026-03-30 的历史路线图，不应直接覆盖当前 PostgreSQL / Neo4j 主链已经接线的实现现状。
- 涉及 PostgreSQL / Neo4j、监控摘要、回填链路或 schema drift 的现态判读，统一沿用 README 的结构化存储状态词汇表：双写完成、仅 PG 模式、待回填、schema drift 待治理。
...
> 历史设计快照（2026-04-08）：本节表格记录的是当时重构设计阶段提出的目标模块与改造判断，不代表 2026-04-14 的当前实现状态。凡是类似“当前无数据库操作代码”“仅有 CLI 入口”“配置声明但无实现”这类描述，都应理解为 2026-04-08 的观察基线；当前真实状态请以较新的架构审计、模块合同与运行回归为准。
...
> 历史目标结构图（2026-04-08）：以下目录树表达的是当时希望收口到的目标结构，而不是当前仓库的逐项实装清单。尤其是 cycle、api、infrastructure、generation 等目录职责，应结合 2026-04-12 之后的 shared runtime / assembler / structured storage 收口结果理解，而不要把本图直接当作 2026-04-14 的现态拓扑。
