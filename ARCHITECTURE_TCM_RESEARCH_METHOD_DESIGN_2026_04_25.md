...
#### 1. 构建图谱增量挖掘守护进程 (Graph Pattern Mining Daemon)
* **目标**：建立周期性（如每日凌晨系统空闲时）拉取 Neo4j 图数据库增量数据的后台任务。
* **操作**：
  - 编写独立的后台队列守护任务（基于 `APScheduler` 或 `Celery`），记录每次挖掘的游标或时间戳，仅计算距上次挖掘点以后的新增图结构。
  - 使用 Cypher 查询如 `(Herb)-[CONTAINS]-(Prescription)-[TREATS]-(Symptom)` 的高频频繁子图组合，快速锁定高共现概率的中医药对与病机规则。
  - 对于挖掘出来的置信度或频次超越阈值的图路径，以 JSON 或内部对象结构送往下游提炼。

#### 2. 知识提鲜与模式提取 (Knowledge Freshness & Pattern Extraction)
* **目标**：将原始的图结构共现频次统计转化成具有医学逻辑的可复用领域经验。
* **操作**：
  - 利用 NetworkX 或 Neo4j 原生图算法（如社区发现、节点中心度 PageRank），识别新的“核心药对”、“高频证候群”及“时代特色方”。
  - 在 `SelfLearningEngine` 中将挖掘出的显式 Pattern 封装为结构化的“学习经验”（Learning Insights）。例如记录：“在明清文献中，温病学派常以连翘配伍金银花治疗温病”。

#### 3. 动态 Few-Shot 样本自动合成 (Dynamic Prompt Few-Shot Synthesis)
* **目标**：把挖掘与提纯到的图谱规律片段反哺给大模型，从预设的死板 Prompt 真正进化为自我指导。
* **操作**：
  - 重构大模型推理的 Prompt 组装层（在 `ReasoningEngine` 或 `PromptTemplateManager` 中），在此预留动态的 Few-Shot 插槽。
  - 当系统处理相似年份流派、相似主诉或相似病机的原始新文献时，自动从上一步构建的“学习经验库”中提取出最具代表性的 2-3 个正例（之前高频验证成功的抽取案例）。
  - 作为动态 Few-Shot Context 拼贴进给 Qwen1.5 大模型的指令任务模板，帮助模型参考系统历来的成熟提取规律，避免模型产生“重新造轮子”的幻觉。

#### 4. 自适调整与打分评估闭环 (Adaptive Tuner & Evaluation Loop)
* **目标**：建立奖惩与进化机制，防止大模型陷入“数据飞轮回音室”中被错误边缘数据带偏。
* **操作**：
  - 引入 `QualityAssessor`，通过设定对角/抽样校验环节对每次使用新 Strategy 生成的实体结果做定期打分与 A/B 评估测试。
  - 如果判定使用动态提取的 Prompt 导致系统整体的信息召回率或逻辑一致性下降，则对该条“学习经验”做降权或直接进黑名单剔除机制。
  - 若应用该策略后的节点抽取表现优于基线（手工打分和图相似图较高），则将其高亮并沉淀固化下发给所有分析线程。

#### 5. 知识固化与人工在环辅助 (Human-in-the-Loop & Strategy Persistence)
* **目标**：让“中医机器脑”的思维过程对研究者科研全过程可视、可管、可控。
* **操作**：
  - 将所有有效凝练的 `LearningStrategy` 与经验池全量持久化双写到 PostgreSQL（存为配置级状态表）及 Neo4j 备查节点。
  - 在 Web Console 或 Dashboard 侧引入“自学习全景洞察卡片”模块。
  - 允许中医研究的领域专家介入，对系统“自我滋养”的科研发现规律（诸如新挖掘的某罕见草药治某病规律）进行定期审视与点赞肯定（赞成即加权、否决即降权）。形成机器与人力完美契合的半自动科研闭环。