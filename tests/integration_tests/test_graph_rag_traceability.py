...
class TestGraphRagTraceability(unittest.TestCase):
    ...
    def test_pulse_difference_analysis(self, mock_llm_service_cls, mock_neo4j_cls):
        ...
        self.assertEqual(
            result.visual_chain_nodes[0], "局方 -> 四物汤 -> 熟地、当归、白芍、川芎"
        )
        self.assertEqual(
            result.visual_chain_nodes[1],
...
class TestTypedGraphRagTraceability(unittest.TestCase):
    def test_typed_evidence_retrieval_returns_traceability_fields(self) -> None:
        mock_driver = MagicMock()
        mock_session = MagicMock()
        mock_driver.driver.session.return_value.__enter__.return_value = mock_session
        mock_session.run.return_value = [
            {
                "node_id": "evidence::cycle-a::ev-1",
                "labels": ["Evidence"],
                "props": {
                    "cycle_id": "cycle-a",
                    "phase": "analyze",
                ...
                "relationship_id": "rel-evidence-claim",
                "neighbor_id": "claim::cycle-a::cl-1",
        ...
        result = GraphRAG(neo4j_driver=mock_driver).retrieve(
            "local",
            query="四物汤",
            asset_type="evidence",
            cycle_id="cycle-a",
        ...
        self.assertEqual(payload["asset_type"], "evidence")
        self.assertEqual(payload["traceability"]["source_phase"], "analyze")
        self.assertEqual(payload["traceability"]["cycle_id"], "cycle-a")
        self.assertEqual(
            payload["traceability"]["node_ids"], ["evidence::cycle-a::ev-1"]
        )
        self.assertEqual(
            payload["traceability"]["relationship_ids"], ["rel-evidence-claim"]
        )
