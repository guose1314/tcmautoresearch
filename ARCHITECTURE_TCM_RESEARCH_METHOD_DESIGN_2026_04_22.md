...
- ✅ J-4 已完成：新增 [src/research/self_refine.py](src/research/self_refine.py)，落 `run_self_refine` / `SelfRefineResult` / `build_self_refine_metadata` + 默认 deterministic scorer/refiner；在 [src/research/phases/hypothesis_phase.py](src/research/phases/hypothesis_phase.py) 与 [src/research/phases/publish_phase.py](src/research/phases/publish_phase.py) 接入自修订并写 `self_refine_quality_delta` 等 8 个 metadata 字段；新增 [tests/unit/test_self_refine.py](tests/unit/test_self_refine.py) 16 通过。详见审计稿 [Card J-4](ARCHITECTURE_TCM_RESEARCH_METHOD_AUDIT_2026_04_20.md#card-j-4)。
- ✅ J-5 已完成：在 [tests/unit/test_architecture_regression_guard.py](tests/unit/test_architecture_regression_guard.py) 新增 `TestGuard41_PhaseJContracts`，7 用例锁定 J-1..J-4 契约；核心子集 1696 通过 / 0 失败 / 1 xfailed / 14 subtests 通过（基线 1673 + 16 J-4 + 7 J-5）。详见审计稿 [Card J-5](ARCHITECTURE_TCM_RESEARCH_METHOD_AUDIT_2026_04_20.md#card-j-5)。
- ✅ Phase J 全部完成；编排器子阶段 wiring 推迟到后续卡片，可进入 Phase K（图谱深化与中医推理）。
