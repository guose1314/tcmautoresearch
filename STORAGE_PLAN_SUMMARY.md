...
本方案将中医古籍研究系统的数据存储从纯 JSON 文件存储升级为 PostgreSQL + Neo4j 混合存储架构，实现：

- ✅ 结构化数据持久化：PostgreSQL 存储元数据、统计、日志、研究结果。
- ✅ 图数据存储：Neo4j 存储知识图谱、实体关系、方剂组成等。
- ✅ 高效查询：支持复杂的图遍历查询和关联分析。
- ✅ 审计追踪：完整的处理日志和版本管理。
- ✅ 学术规范：符合 T/C IATCM 098-2023 标准。
...
> 历史设计目标图（2026-03-28）：以下 ASCII 架构块展示的是当时规划中的存储增强版系统分层，用于保留方案摘要，不应直接视为当前默认实现结构。
...
> 历史设计目标图（2026-03-28）：以下 ASCII 数据流块描述的是当时计划中的入库链路与分库写入步骤，用于说明方案目标，不等同于当前运行时的真实调用细节。
...
| **STORAGE_DEPLOYMENT.md** | PostgreSQL、Neo4j 安装和初始化 | 部署指南 |
| **STORAGE_QUERIES.md** | PostgreSQL 和 Neo4j 常用查询、Python 示例 | 查询参考 |
...
| PostgreSQL 表总大小 | 10-100GB | 按 100K 文档计 |
...
2. **并发写入**：PostgreSQL 默认池大小 10，高并发需调整。
3. **图查询深度**：Neo4j 深度查询（>5 层）性能下降。
...
3. **[STORAGE_QUERIES.md](STORAGE_QUERIES.md)** - SQL 和 Cypher 查询示例
...
本方案遵循 MIT 许可证，符合 T/C IATCM 098-2023 标准。
