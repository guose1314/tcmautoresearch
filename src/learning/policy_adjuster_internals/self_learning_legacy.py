...
class SelfLearningEngine(BaseModule):
    ...
    def register_graph_insight(self, insight_data: Dict[str, Any]) -> None:
        """
        接收图谱挖掘(Graph Pattern Mining)产生的高频模式，转化为 Learning Insights 记录。
        这满足了 Phase 4: Regular self-learning loop online (Step 2: Knowledge Freshness)。
        """
        if not insight_data:
            return
    ...
    def get_dynamic_few_shot_context(self, limit: int = 3) -> tuple[str, list[str]]:
        ...
        返回 dynamic_few_shot_str 以及对应使用的 task_ids（用于后续的降权/固化闭环）
        """
        insights = []
        task_ids = []
        ...
        for idx, insight in enumerate(insights, 1):
            ...
            reasoning = "N/A"
            if isinstance(insight, dict) and "frequent_paths" in insight:
                paths = insight["frequent_paths"]
                reasoning = f"Found {len(paths)} frequent evidence paths in graph."
    ...
    def apply_few_shot_feedback(self, task_ids: list[str], score: float) -> None:
        """
        自适调整与打分评估闭环 (Adaptive Tuner & Evaluation Loop)
        对于 LLM 利用 prompt 产生新结果的情况，评估其打分。
        表现差 (score < 0.5): 降权甚至剔除
        表现好 (score >= 0.8): 增加固化分数
        """
        if not task_ids:
            return
