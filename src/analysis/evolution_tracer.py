...
class GraphEvolutionTracer:
    def __init__(self, neo4j_driver: Neo4jDriver, llm_engine: Optional[CachedLLMService] = None):
        self.neo4j = neo4j_driver
        self.llm = llm_engine or CachedLLMService()

    def trace_formula_evolution(self, formula_name: str) -> PulseDifferenceAnalysisResult:
        """
        ...
        analysis_payload, used_task_ids = self._invoke_llm_for_analysis(formula_name, graph_data)
        ...
        try:
            ...
            if engine_info and getattr(engine_info, 'instance', None):
                engine_info.instance.apply_few_shot_feedback(used_task_ids, quality_score)
        ...
        return PulseDifferenceAnalysisResult(
            ...
            citations=self._map_citations(analysis_payload.get("citations", []), graph_data)
        )
    ...
    def _invoke_llm_for_analysis(self, formula_name: str, graph_data: List[Dict[str, Any]]) -> tuple[Dict[str, Any], list[str]]:
        """
        利用大模型融合 Graph Context 进行分析。
        要求大模型在输出结论必须带文献标题。
        """
        graph_text = "\n".join([f"- 出处: 《{d['literature']}》 | 药味组合: {','.join(d['herbs'])}" for d in graph_data])
        ...
        try:
            ...
            if engine_info and getattr(engine_info, 'instance', None):
                ...
                dynamic_few_shot, task_ids = engine_info.instance.get_dynamic_few_shot_context(limit=2)
        except Exception as e:
...
"""
        ...
        try:
            raw_response = self.llm._request_completion("gpt-3.5-turbo", [{"role": "user", "content": prompt}], temperature=0.1)
            # 简单剥离 JSON block (容错性)
    ...
    def _map_citations(self, citations: List[str], graph_data: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        mapped = []
