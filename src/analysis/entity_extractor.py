...
class AdvancedEntityExtractor(BaseModule):
    ...
    def __init__(self, config: Dict[str, Any] | None = None):
        ...
        self.term_sense_disambiguator = TermSenseDisambiguator()
    ...
    def _do_execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            entities = self._extract_entities(processed_text, metadata=metadata)
            statistics = self._calculate_statistics(entities)
            statistics["sense_candidate_count"] = sum(
                1 for item in entities if item.get("sense_candidates")
            )
            statistics["sense_resolved_count"] = sum(
                1 for item in entities if item.get("sense_id")
            )
    ...
    def _extract_entities(
        ...
        for word in self.term_sense_disambiguator.supported_terms:
            for start_pos, end_pos in self._iter_word_matches(text, word):
                if self._is_position_overlapped(matched_positions, start_pos, end_pos):
                    continue
                entities.append(self._build_seed_term_record(word, start_pos, end_pos))
                matched_positions.update(range(start_pos, end_pos))
        ...
        entities = self.term_sense_disambiguator.disambiguate_entities(
            entities,
            text,
            dynasty=self._resolve_dynasty_hint(metadata or {}),
        )
    ...
    def _is_position_overlapped(
        self, matched_positions: Set[int], start_pos: int, end_pos: int
    ) -> bool:
        """判断目标区间是否与已匹配区间重叠。"""
        return any(pos in matched_positions for pos in range(start_pos, end_pos))

    def _build_entity_record(
        self, word: str, start_pos: int, end_pos: int
    ) -> Dict[str, Any]:
    ...
    def _build_seed_term_record(
        self, word: str, start_pos: int, end_pos: int
    ) -> Dict[str, Any]:
        """Build a record for high-frequency polysemous seed terms."""
        return {
            "name": word,
            "type": self.term_sense_disambiguator.default_category(word),
            "confidence": 0.78,
            "position": start_pos,
            "end_position": end_pos,
            "length": len(word),
            "source": "term_sense_seed",
        }

    def _resolve_dynasty_hint(self, metadata: Dict[str, Any]) -> str | None:
        for key in ("dynasty", "period", "era"):
            value = metadata.get(key)
            if value:
                return str(value)
        for key in ("version_metadata", "canonical_identity", "document_metadata"):
            nested = metadata.get(key)
            if isinstance(nested, dict):
                for nested_key in ("dynasty", "period", "era"):
    ...
    def load_external_lexicon(self, filepath: str, word_type: str = "common") -> int:
        ...
        if count > 0:
            self.logger.info(
                f"已加载 {count} 个外部 {word_type} 词汇，词典总规模: {self.lexicon.get_vocab_size()}"
            )
        return count
