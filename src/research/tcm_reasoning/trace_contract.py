...
class TCMReasoningPremise:
    ...
    note: str = ""
    claim_id: str = ""  # GraphRAG / EvidenceClaim 追踪 ID
    source_phase: str = ""  # observe / analyze / publish 等来源阶段
    confidence: float = 0.0

    def premise_ref(self) -> str:
        return (
            self.claim_id or self.source_ref or f"{self.premise_kind}:{self.canonical}"
        )

    def to_dict(self) -> Dict[str, Any]:
        payload = asdict(self)
        payload["confidence"] = _clamp_unit(self.confidence)
        payload["premise_ref"] = self.premise_ref()
        return payload
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "TCMReasoningPremise":
        ...
        return cls(
            ...
            claim_id=_as_text(d.get("claim_id")),
            source_phase=_as_text(d.get("source_phase")),
...
class TCMReasoningStep:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "premise_refs": _unique_texts(p.premise_ref() for p in self.premises),
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "TCMReasoningStep":
        ...
        return cls(
            ...
            premises=[
                TCMReasoningPremise.from_dict(p)
