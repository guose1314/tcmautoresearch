...
_TEMPLATES: Dict[StudyType, Dict[str, Any]] = {
    StudyType.RCT: {
        ...
        "default_visit_schedule": [
            {
                "timepoint": "筛选期",
                "assessments": ["诊断确认", "纳排评估", "知情同意"],
            },
            {"timepoint": "基线", "assessments": ["证候评分", "实验室指标", "随机化"]},
            {"timepoint": "治疗中", "assessments": ["依从性", "不良事件", "疗效趋势"]},
            {
                "timepoint": "末次访视/随访",
                "assessments": ["主要结局", "安全性", "复发情况"],
            },
        ],
        "default_data_collection_plan": [
            "eCRF 录入",
            "证候量表评分",
            "实验室/影像检查",
            "安全性随访",
    ...
    StudyType.COHORT: {
        ...
        "default_visit_schedule": [
            {
                "timepoint": "基线",
                "assessments": ["暴露确认", "共变量收集", "结局定义"],
            },
            {
                "timepoint": "随访期",
                "assessments": ["结局事件", "依从性", "混杂因素更新"],
            },
            {
                "timepoint": "结题",
                "assessments": ["结局核验", "统计分析", "敏感性分析"],
            },
        ],
        "default_data_collection_plan": [
            "随访表",
...
def _estimate_sample_size(
    alpha: float = 0.05, power: float = 0.80, effect_size: float = 0.5
) -> int:
    """基于 Cohen's d 简化估算两组比较所需总样本量。"""
    return _estimate_continuous_sample_size(
        alpha=alpha, power=power, effect_size=effect_size
    )
...
def _estimate_case_control_sample_size(
    ...
    return _estimate_binary_sample_size(
        alpha=alpha, power=power, control_rate=p0, target_rate=p1
    )
...
def _infer_outcome_type(
    ...
    if study_type == StudyType.NETWORK_PHARMACOLOGY or any(
        keyword in outcome_text for keyword in _MECHANISTIC_OUTCOME_KEYWORDS
    ):
        return "mechanistic"
...
class ExperimentDesigner(BaseModule):
    ...
    def __init__(self, config: Dict[str, Any] | None = None, llm_engine: Any = None):
        ...
        self.llm_engine = (
            llm_engine
            or self.config.get("llm_engine")
            or self.config.get("llm_service")
        )
    ...
    def _do_execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        protocol = self.design_study(
            ...
            objective=_coerce_text(
                context.get("objective") or context.get("research_objective")
            ),
            ...
            outcome=_coerce_text(
                context.get("outcome") or context.get("primary_outcome")
            ),
    ...
    def design_study(
        ...
        *,
        ...
        protocol = self._ensure_protocol_completeness(
            protocol, stype, hypothesis, template
        )
    ...
    def _build_template_protocol(
        ...
        *,
        ...
        protocol = StudyProtocol(
            ...
            visit_schedule=self._build_visit_schedule(
                template.get("default_visit_schedule")
            ),
            blinding=_coerce_text(template.get("blinding")),
            randomization=_coerce_text(template.get("randomization")),
            data_collection_plan=_ensure_text_list(
                template.get("default_data_collection_plan")
            ),
            statistical_plan=_coerce_text(template.get("statistical_plan")),
            analysis_populations=_ensure_text_list(
                template.get("default_analysis_populations")
            ),
        ...
        for item in raw_arms:
            ...
            arm = StudyArm(
                ...
                allocation_ratio=max(
                    0.1, _coerce_float(item.get("allocation_ratio"), 1.0)
                ),
            )
            if arm.name:
                arms.append(arm)
    ...
    def _generate_protocol_payload(
        ...
        *,
        ...
        prompt = self._build_llm_prompt(
            hypothesis, study_type, protocol, additional_context
        )
        dossier_sections = {
            "hypothesis": hypothesis,
            "study_type": study_type.value,
        ...
        try:
            gateway_result = generate_with_gateway(
                llm_engine,
                prompt,
                self.PROTOCOL_SYSTEM_PROMPT,
                prompt_version="experiment_designer.protocol_design@v1",
                phase="experiment",
                purpose="protocol_design",
                task_type="protocol_design",
                json_output=True,
                metadata={
                    "prompt_name": "experiment_designer.protocol_design",
                    "response_format": "json",
                    "dossier_sections": dossier_sections,
                },
            )
            planner = dict(gateway_result.metadata.get("planned_call") or {})
            self._last_small_model_plan = planner
            self._last_llm_cost_report = gateway_result.llm_cost_report
            self._last_fallback_path = planner.get("fallback_path")
    ...
    def _build_llm_prompt(
        ...
        context_digest = {
            "research_objective": additional_context.get("research_objective")
            or additional_context.get("objective"),
    ...
    def _merge_protocol_payload(
        self, protocol: StudyProtocol, payload: Mapping[str, Any]
    ) -> StudyProtocol:
        protocol.title = _coerce_text(payload.get("title")) or protocol.title
        protocol.objective = (
            _coerce_text(payload.get("objective")) or protocol.objective
        )
        protocol.background_rationale = (
            _coerce_text(payload.get("background_rationale"))
            or protocol.background_rationale
        )
        protocol.primary_outcome = (
            _coerce_text(payload.get("primary_outcome")) or protocol.primary_outcome
        )
        protocol.secondary_outcomes = (
            _ensure_text_list(payload.get("secondary_outcomes"))
            or protocol.secondary_outcomes
        )
        protocol.blinding = _coerce_text(payload.get("blinding")) or protocol.blinding
        protocol.randomization = (
            _coerce_text(payload.get("randomization")) or protocol.randomization
        )
        protocol.statistical_plan = (
            _coerce_text(payload.get("statistical_plan")) or protocol.statistical_plan
        )
        protocol.duration = _coerce_text(payload.get("duration")) or protocol.duration
        protocol.safety_monitoring = (
            _coerce_text(payload.get("safety_monitoring")) or protocol.safety_monitoring
        )
        protocol.ethical_considerations = (
            _coerce_text(payload.get("ethical_considerations"))
            or protocol.ethical_considerations
        )

        pico_payload = payload.get("pico")
        if isinstance(pico_payload, Mapping):
            protocol.pico.population = (
                _coerce_text(pico_payload.get("population")) or protocol.pico.population
            )
            protocol.pico.intervention = (
                _coerce_text(pico_payload.get("intervention"))
                or protocol.pico.intervention
            )
            protocol.pico.comparison = (
                _coerce_text(pico_payload.get("comparison")) or protocol.pico.comparison
            )
            protocol.pico.outcome = (
                _coerce_text(pico_payload.get("outcome")) or protocol.pico.outcome
            )

        eligibility_payload = payload.get("eligibility")
        if isinstance(eligibility_payload, Mapping):
            protocol.eligibility.inclusion = (
                _ensure_text_list(eligibility_payload.get("inclusion"))
                or protocol.eligibility.inclusion
            )
            protocol.eligibility.exclusion = (
                _ensure_text_list(eligibility_payload.get("exclusion"))
                or protocol.eligibility.exclusion
            )
        ...
        protocol.procedures = (
            _ensure_text_list(payload.get("procedures")) or protocol.procedures
        )
        protocol.data_collection_plan = (
            _ensure_text_list(payload.get("data_collection_plan"))
            or protocol.data_collection_plan
        )
        protocol.analysis_populations = (
            _ensure_text_list(payload.get("analysis_populations"))
            or protocol.analysis_populations
        )
        protocol.risk_management = (
            _ensure_text_list(payload.get("risk_management"))
            or protocol.risk_management
        )
        protocol.design_notes = (
            _ensure_text_list(payload.get("design_notes")) or protocol.design_notes
        )
        return protocol
    ...
    def _build_sample_size_estimation(
        ...
        *,
        ...
        resolved_alpha = _clamp(
            _coerce_float(sample_payload.get("alpha"), alpha), 0.001, 0.2
        )
        resolved_power = _clamp(
            _coerce_float(sample_payload.get("power"), power), 0.5, 0.99
        )
        resolved_effect_size = _coerce_float(
            effect_size
            if effect_size is not None
            else sample_payload.get("effect_size"),
            _coerce_float(template.get("default_effect_size"), 0.5),
        )
        allocation_ratio = max(
            0.2, _coerce_float(sample_payload.get("allocation_ratio"), 1.0)
        )
        default_dropout = (
            0.15
            if study_type in (StudyType.RCT, StudyType.COHORT, StudyType.CASE_CONTROL)
            else 0.0
        )
        dropout_rate = _clamp(
            _coerce_float(sample_payload.get("dropout_rate"), default_dropout), 0.0, 0.4
        )
        ...
        if outcome_type == "binary":
            control_rate = _clamp(
                _coerce_float(
                    sample_payload.get("control_rate")
                    or sample_payload.get("baseline_rate"),
                    0.50,
                ),
                0.05,
                0.95,
            )
            absolute_improvement = _coerce_float(
                sample_payload.get("absolute_improvement"), 0.15
            )
            target_rate = _clamp(
                _coerce_float(
                    sample_payload.get("target_rate"),
                    control_rate + absolute_improvement,
                ),
                0.05,
                0.99,
            )
        ...
        if outcome_type == "case_control":
            control_exposure_rate = _clamp(
                _coerce_float(sample_payload.get("control_exposure_rate"), 0.30),
                0.05,
                0.95,
            )
    ...
    def _ensure_protocol_completeness(
        ...
        protocol.background_rationale = protocol.background_rationale or _coerce_text(
            template.get("background_rationale")
        )
        ...
        if not protocol.visit_schedule:
            protocol.visit_schedule = self._build_visit_schedule(
                template.get("default_visit_schedule")
            )
        if not protocol.data_collection_plan:
            protocol.data_collection_plan = _ensure_text_list(
                template.get("default_data_collection_plan")
            )
        if not protocol.analysis_populations:
            protocol.analysis_populations = _ensure_text_list(
                template.get("default_analysis_populations")
            )
        if not protocol.risk_management:
            protocol.risk_management = _ensure_text_list(
                template.get("default_risk_management")
            )
    ...
    def _resolve_study_type(raw: str | StudyType) -> StudyType:
        ...
        for member in StudyType:
            if member.value == normalized or member.name.lower() == normalized:
                return member
        raise ValueError(
            f"不支持的研究类型: {raw!r}，可选: {[item.value for item in StudyType]}"
        )
