...
def infer_methodology_tag(source_gap_type: str) -> str:
    ...
    if any(
        token in gap
        for token in ("version", "variant", "witness", "lineage", "philolog")
    ):
        return "philology"
    if any(
        token in gap
        for token in ("composition", "classification", "taxonomy", "category")
    ):
...
"""
    ...
    def __init__(
        ...
        self.minimum_evidence_support = float(
            self.config.get("minimum_evidence_support", 0.0)
        )
        self.active_confidence_threshold = float(
            self.config.get("active_confidence_threshold", 0.65)
        )
        self.validation_confidence_threshold = float(
            self.config.get("validation_confidence_threshold", 0.8)
        )
        self.system_prompt = str(
            self.config.get("system_prompt") or self.DEFAULT_SYSTEM_PROMPT
        )
    ...
    def _do_execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            hypotheses = self.generate_hypotheses(
                knowledge_gap=knowledge_gap, context=context
            )
            ranked = self._enrich_hypotheses(
                self.rank_hypotheses(hypotheses, context), context
            )
        finally:
            self.knowledge_graph = previous_graph

        top_hypothesis = ranked[0] if ranked else None
        used_llm_generation = any(
            item.generation_mode in ("llm", "kg_enhanced") for item in ranked
        )
        ...
        return {
            ...
            "metadata": {
                ...
                "selected_hypothesis_id": top_hypothesis.hypothesis_id
                if top_hypothesis
                else "",
                ...
                "research_direction": top_hypothesis.title
                if top_hypothesis
                else str(context.get("research_objective") or ""),
    ...
    def generate_hypotheses(
        ...
        if not tier_decision.should_use_llm:
            ...
            return self._enrich_hypotheses(
                self.rank_hypotheses(
                    self._generate_with_rules(gap, prepared_context), prepared_context
                ),
                prepared_context,
            )
        ...
        # --- 原有 LLM 路径 ---
        if active_llm_engine is not None:
            llm_hypotheses = self._generate_with_llm(
                gap, prepared_context, active_llm_engine
            )
            if llm_hypotheses:
                return self._enrich_hypotheses(
                    self.rank_hypotheses(llm_hypotheses, prepared_context),
                    prepared_context,
                )
        return self._enrich_hypotheses(
            self.rank_hypotheses(
    ...
    def _resolve_hypothesis_score_weights(
        ...
        return {key: round(value / total_weight, 6) for key, value in weights.items()}
    ...
    def _resolve_status_thresholds(
        ...
        active_threshold = round(
            max(active_threshold, min(0.85, confidence_threshold)), 4
        )
        validated_threshold = round(
            max(validated_threshold, min(0.95, active_threshold + 0.1)), 4
        )
        return validated_threshold, active_threshold
    ...
    def _parse_llm_response(
        ...
        *,
        ...
        for item in payload:
            ...
            evidence_support = self._normalize_score(
                item.get("evidence_support"), default=0.65
            )
            validation_plan = str(
            ...
            if returned_gap_type and (
                allowed_gap_types is None or returned_gap_type in allowed_gap_types
            ):
                source_gap_type = returned_gap_type
            else:
                source_gap_type = gap["gap_type"]
            source_entities = (
                self._ensure_text_list(item.get("source_entities")) or gap["entities"]
            )
            hypotheses.append(
    ...
    def _build_kg_structure_summary(
        ...
        try:
            if hasattr(graph, "entity_count"):
                parts.append(
                    f"图谱节点数: {graph.entity_count}，边数: {graph.relation_count}"
                )
        except Exception:
            pass
        ...
        for gap in gaps[:5]:
            ...
            try:
                ...
                if neighbor_count > 0:
                    neighbor_names = [
                        str(n) for n in subgraph.nodes if str(n) != gap.entity
                    ][:5]
                    parts.append(
    ...
    def _generate_with_rules(
        ...
        while len(deduplicated) < min(self.max_hypotheses, 3):
            deduplicated.append(
                self._build_backfill_hypothesis(gap, context, len(deduplicated) + 1)
            )
        return deduplicated[: max(self.max_hypotheses, 3)]

    def _hypotheses_from_gap_type(
        self, gap: Dict[str, Any], context: Dict[str, Any]
    ) -> List[Hypothesis]:
        entity = gap["entity"]
        ...
        if gap_type == "missing_direct_relation":
            return [
                self._build_hypothesis(
                    ...
                    novelty=0.84,
                    feasibility=0.73,
                    evidence_support=self._estimate_evidence_support(
                        gap, context, bonus=0.08
                    ),
                ),
                self._build_hypothesis(
    ...
    def _hypotheses_from_graph_structure(
        self, gap: Dict[str, Any], context: Dict[str, Any]
    ) -> List[Hypothesis]:
        ...
        if subgraph.number_of_nodes() >= 3:
            neighbor_names = [
                str(node) for node in subgraph.nodes if str(node) != gap["entity"]
            ][:3]
            return [
                self._build_hypothesis(
                    ...
                    feasibility=0.71,
                    evidence_support=self._estimate_evidence_support(
                        gap, context, bonus=0.07
                    ),
                )
            ]
        return []

    def _hypotheses_from_context(
        self, gap: Dict[str, Any], context: Dict[str, Any]
    ) -> List[Hypothesis]:
        evidence = (
        ...
        return [
            self._build_hypothesis(
                ...
                feasibility=0.79,
                evidence_support=self._estimate_evidence_support(
                    gap, context, bonus=0.09
                ),
            )
        ]

    def _normalize_gap(
        self, knowledge_gap: Any, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if isinstance(knowledge_gap, dict):
            entity = str(
                knowledge_gap.get("entity") or knowledge_gap.get("source") or "未知实体"
            )
            entities = knowledge_gap.get("entities")
            if not isinstance(entities, list) or not entities:
                related = [
                    knowledge_gap.get("source"),
                    knowledge_gap.get("target"),
                    entity,
                ]
            ...
            return {
                ...
                "description": str(
                    knowledge_gap.get("description") or "未命名知识缺口"
                ),
    ...
    def _evaluate_compute_tier(
        self, context: Dict[str, Any], gap: Dict[str, Any]
    ) -> TierDecision:
        ...
        evidence = {
            ...
            "evidence_items": len(context.get("observations") or [])
            + len(context.get("findings") or []),
    ...
    def _resolve_llm_engine(self, context: Dict[str, Any]) -> Any:
        ...
        if "use_llm_generation" not in context:
            if "hypothesis_use_llm_generation" in strategy and not strategy.get(
                "hypothesis_use_llm_generation"
            ):
                return None
            if "use_llm_generation" in strategy and not strategy.get(
                "use_llm_generation"
            ):
                return None
        return (
            context.get("llm_service") or context.get("llm_engine") or self.llm_engine
        )
    ...
    def _build_hypothesis_dossier_sections(
        ...
        literature_titles = [
            str(item).strip()
            for item in (context.get("literature_titles") or [])
            if str(item).strip()
        ]
    ...
    def _estimate_evidence_support(
        ...
        severity_bonus = {"high": 0.04, "medium": 0.02, "low": 0.0}.get(
            gap["severity"], 0.01
        )
    ...
    def _build_backfill_hypothesis(
        ...
        return self._build_hypothesis(
            ...
            keywords=[entity]
            + self._extract_keywords(self._context_strings(context))[:2],
            novelty=0.63,
            feasibility=0.7,
            evidence_support=self._estimate_evidence_support(gap, context),
        )
    ...
    def _enrich_hypotheses(
        ...
        contradiction_signals = self._ensure_text_list(context.get("contradictions"))[
            :3
        ]
        ...
        for item in hypotheses:
            mechanism_completeness = self._estimate_mechanism_completeness(
                item, context
            )
            item.domain = item.domain or domain
            item.supporting_signals = item.supporting_signals or supporting_signals
            item.contradiction_signals = (
                item.contradiction_signals or contradiction_signals
            )
            item.scores = {
                **item.scores,
            ...
            if (
                item.evidence_support < minimum_evidence_support
    ...
    def _estimate_relevance(
        ...
        overlap = sum(
            1 for keyword in hypothesis.keywords if keyword in objective_terms
        )
        return round(min(1.0, 0.5 + overlap * 0.12), 4)

    def _estimate_mechanism_completeness(
        self, hypothesis: Hypothesis, context: Dict[str, Any]
    ) -> float:
        reasoning_summary = context.get("reasoning_summary") or {}
        knowledge_patterns = (
            context.get("knowledge_patterns")
            or reasoning_summary.get("knowledge_patterns")
            or {}
        )
        inference_confidence = float(
            context.get("inference_confidence")
            or reasoning_summary.get("inference_confidence")
            or 0.0
        )
        common_entities = [
            str(item)
        ...
        shared_efficacies = [
            str(item)
            for item in (knowledge_patterns.get("most_shared_efficacies") or [])
            if str(item).strip()
        ]
        ...
        keyword_hits = sum(
            ...
            if keyword in common_entities or keyword in shared_efficacies
        )
        entity_hits = sum(
            1
            for entity in hypothesis.source_entities
            if entity in common_entities
            or any(entity in values for values in grouped_entities.values())
        )
        path_bonus = 0.0
        if len(hypothesis.source_entities) >= 2:
            paths = self.knowledge_graph.query_path(
                hypothesis.source_entities[0], hypothesis.source_entities[-1]
            )
            if paths:
