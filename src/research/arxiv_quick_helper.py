...
def _fetch_arxiv_metadata(arxiv_id: str) -> Dict[str, str]:
    ...
    if BeautifulSoup is None:
        raise RuntimeError(
            "BeautifulSoup not installed. Install via: pip install beautifulsoup4"
        )

    metadata = {"title": "", "authors": "", "publish_date": "", "abstract_en": ""}
...
def _translate_abstract_with_llm(abstract: str, target_lang: str, llm_engine) -> str:
    """
    使用 LLM 翻译摘要
...
Translated abstract (in {target_lang}):"""
        ...
        if hasattr(llm_engine, "generate"):
            ...
            gateway_result = generate_with_gateway(
                llm_engine,
                prompt,
                prompt_version="arxiv_quick_helper.abstract_translation@v1",
                phase="literature",
                purpose="abstract_translation",
                task_type="translation",
                metadata={
                    "prompt_name": "arxiv_quick_helper.abstract_translation",
        ...
        if enable_translation and result.abstract_en:
            try:
                result.abstract_zh = _translate_abstract_with_llm(
                    result.abstract_en, target_lang, llm_engine
                )
            except Exception as e:
                # 翻译失败不中断整个流程
...
def persist_arxiv_result_to_dual_storage(
    result: ArxivQuickHelperResult, db_engine=None, neo4j_session=None
) -> bool:
    """
